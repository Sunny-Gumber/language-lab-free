// Language Lab Free — V5.3.1 reliable multi-device cloud sync
(function(){
  ...
  function writeLocal(data){localStorage.setItem(STORAGE_KEY,JSON.stringify(data));try{if(typeof state!=='undefined')state=data;}catch{}}
  ...
  function refreshUI(){try{if(typeof renderTop==='function')renderTop();if(typeof renderLanguages==='function')renderLanguages();if(typeof renderProgress==='function'&&document.getElementById('progressTab')?.classList.contains('active'))renderProgress();}catch(e){console.debug('[Language Lab Free] UI refresh skipped',e)}}
  ...
  async function fetchCloud(){
    const [p,l,a]=await Promise.all([
      client.from('profiles').select('id,total_xp,current_streak,longest_streak,last_study_date,selected_language,timezone,updated_at').eq('id',user.id).maybeSingle(),
      ...
      client.from('study_activity').select('activity_date,xp_earned,study_minutes,updated_at').eq('user_id',user.id).order('activity_date',{ascending:false}).limit(60)
    ]);
  ...
  async function pushState(s,remote){
    ...
    const rows=Object.entries(s.languages||{}).map(([code,v])=>{const r=remoteByLang[code]||{};return {user_id:user.id,language_code:code,current_unit:Math.max(Number(v.currentUnit||0),Number(r.current_unit||0)),current_lesson:Math.max(Number(v.currentLesson||0),Number(r.current_lesson||0)),mastery:mergeMastery(v.mastery||{},r.mastery||{}),favorites:union(v.favorites||[],r.favorites||[]),writing_attempts:Math.max(Number(v.writes||0),Number(r.writing_attempts||0)),quiz_correct:Math.max(Number(v.quizCorrect||0),Number(r.quiz_correct||0)),quiz_total:Math.max(Number(v.quizTotal||0),Number(r.quiz_total||0)),xp:Math.max(Number(v.xp||0),Number(r.xp||0))};});
    ...
    if(profile.last_study_date===today){
      const previousDays=(remote.activity||[]).filter(x=>x.activity_date!==today).reduce((sum,x)=>sum+Number(x.xp_earned||0),0);
      const earnedToday=Math.max(0,Number(profile.total_xp||0)-previousDays);
      const current=(remote.activity||[]).find(x=>x.activity_date===today);
      const {error:ae}=await client.from('study_activity').upsert({user_id:user.id,activity_date:today,xp_earned:Math.max(Number(current?.xp_earned||0),earnedToday),study_minutes:Number(current?.study_minutes||0)},{onConflict:'user_id,activity_date'});if(ae)throw ae;
    }
  }
