...
test('guest V14 progress can be imported into a newly signed-in account',async({page},testInfo)=>{
  ...
  await expect.poll(()=>page.evaluate(()=>window.__mockBackend.events.some(event=>event.metadata?.mode==='v14-retrieval'))).toBe(true);
});
...
test('event progress and exact detailed-lesson position restore on a fresh second device',async({page,browser},testInfo)=>{
  ...
  await page.locator('[data-tab="explore"]').click();await page.locator('[data-v14-tool="learn"]').click();
  ...
  await page.locator('#learnTab [data-v14-back]').click();await page.locator('[data-tab="review"]').click();await page.locator('[data-v14-tool="cards"]').click();
  ...
  await secondPage.locator('[data-tab="explore"]').click();await secondPage.locator('[data-v14-tool="learn"]').click();await expect(secondPage.locator('#unitSelect')).toHaveValue('2');
