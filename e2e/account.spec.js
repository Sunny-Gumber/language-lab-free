...
test('signed-in onboarding, language management and sign-out work end to end',async({page})=>{
  ...
  expect(profile.primary_language).toBe('es');
  expect(profile.enabled_languages).toEqual(['es']);
  expect(profile.audio_preference).toBe('female');
  expect(profile.onboarding_completed).toBe(true);

  await page.locator('#addLanguageBtn').click();
  const japanese=page.locator('.manager-language').filter({hasText:'Japanese'});
  await japanese.getByRole('button',{name:'Add'}).click();
  await japanese.getByRole('button',{name:'Make primary'}).click();
...
test('event progress and exact course position restore on a fresh second device',async({page,browser},testInfo)=>{
  ...
  await blockExternal(page);
  await installMockSupabase(page,{profile:accountProfile});
  await page.goto('/');
  await waitForBoot(page);
  await expect(page.locator('#onboardingOverlay')).toHaveClass(/hidden/);

  await page.locator('[data-language="ja"]').click();
  await page.locator('#unitSelect').selectOption('2');
  await expect(page.locator('#unitSelect')).toHaveValue('2');
  ...
  await installMockSupabase(secondPage,snapshot);
  await secondPage.goto('/');
  await waitForBoot(secondPage);
  await expect(secondPage.locator('#topXp')).toHaveText('4');
  await secondPage.locator('[data-language="ja"]').click();
  await expect(secondPage.locator('#unitSelect')).toHaveValue('2');
  const secondState=await secondPage.evaluate(()=>window.LanguageLab.getState());
  expect(secondState.events.some(event=>event.xpDelta===4&&event.skill==='recall')).toBe(true);
