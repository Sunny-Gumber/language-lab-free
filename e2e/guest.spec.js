...
test('guest practice persists in IndexedDB, survives reload and resets cleanly',async({page})=>{
  await page.goto('/');await waitForBoot(page);
  await page.locator('[data-language="ja"]').click();await expect(page.locator('#courseName')).toHaveText('Japanese');await expect(page.locator('#learnTab')).toHaveClass(/active/);
