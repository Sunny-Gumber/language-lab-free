...
test('Hindi pronunciation module is cached and works offline in V15.1',async({page,context})=>{
  ...
  await expect.poll(()=>page.evaluate(async()=>Boolean(await caches.open('language-lab-free-v15-1').then(cache=>cache.match('./src/pronunciation-hi.js'))))).toBe(true);
