...
test('PWA caches pinned runtime and V13.1 adaptive learning assets for fully offline start',async({page,context})=>{
  ...
  const cached=await page.evaluate(async url=>{
    const cache=await caches.open('language-lab-free-v13-1');
    return{
      runtime:Boolean(await cache.match(url)),
      homeCss:Boolean(await cache.match('./home-v12.css')),
      journeyCss:Boolean(await cache.match('./journey-v13.css')),
      journeyJs:Boolean(await cache.match('./src/journey.js')),
      sessionJs:Boolean(await cache.match('./src/session.js'))
    };
  },PINNED_SUPABASE);
  expect(cached.runtime).toBe(true);expect(cached.homeCss).toBe(true);expect(cached.journeyCss).toBe(true);expect(cached.journeyJs).toBe(true);expect(cached.sessionJs).toBe(true);
