...
test('PWA caches pinned runtime and V15.1 interleaved planner assets for offline start',async({page,context})=>{
  ...
  const cached=await page.evaluate(async url=>{
    const cache=await caches.open('language-lab-free-v15-1');
    return{
      ...
      coursePackJs:Boolean(await cache.match('./src/course-pack.js')),
      activityEngineJs:Boolean(await cache.match('./src/activity-engine.js')),
