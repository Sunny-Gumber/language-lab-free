import{courses,courseCodes,getCourse}from'./data.js';
...
export class AccountUi{
  ...
  bind(){
    ...
    document.addEventListener('keydown',event=>{
      if(event.key==='Escape')document.querySelectorAll('.overlay:not(.hidden)').forEach(overlay=>closeOverlay(overlay.id));
    });
  ...
  openAccount(){
    ...
    if(isSignedIn()){
      ...
      $('accountLanguagesBtn').addEventListener('click',()=>{
        closeOverlay('authOverlay');
        this.openLanguages('manage');
      });
      ...
      body.innerHTML=`
        ...
        <p class="tiny muted">You can also close this window and continue as Guest. Guest learning stays on this browser.</p>`;
  ...
  checkOnboarding(){
    ...
    if(!isSignedIn()){
      closeOverlay('onboardingOverlay');
      return;
    }
    ...
    this.renderOnboardingLanguages();
    this.renderOnboardingAudio();
  ...
  renderOnboardingLanguages(){
    $('onboardingLanguages').innerHTML=courses.map(course=>`
      <button class="language-choice ${course.id===this.onboardingLanguage?'active':''}" data-onboard-language="${course.id}" type="button">
        <span>${course.flag}</span><b>${escapeHtml(course.name)}</b>
      </button>`).join('');
    $('onboardingLanguages').querySelectorAll('[data-onboard-language]').forEach(button=>button.addEventListener('click',()=>{
      this.onboardingLanguage=button.dataset.onboardLanguage;
      this.renderOnboardingLanguages();
    }));
  }

  renderOnboardingAudio(){
    $('onboardingAudio').querySelectorAll('[data-onboard-audio]').forEach(button=>button.classList.toggle('active',button.dataset.onboardAudio===this.onboardingAudio));
  }

  async saveOnboarding(){
    if(!isSignedIn())return;
    const button=$('onboardingSaveBtn');
    button.disabled=true;
    button.textContent='Saving…';
    $('onboardingStatus').textContent='';
    try{
      updatePrefs({
        primaryLanguage:this.onboardingLanguage,
        enabledLanguages:[this.onboardingLanguage],
        audioPreference:this.onboardingAudio,
        onboardingCompleted:true
      });
      await syncNow('onboarding');
      closeOverlay('onboardingOverlay');
    }catch(error){
      $('onboardingStatus').textContent=error.message||'Could not save yet. Please try again.';
    }finally{
  ...
  openLanguages(mode='manage'){
    if(!isSignedIn())return;
    $('languagesModalTitle').textContent=mode==='add'?'Add another language':'Manage languages';
    this.renderLanguageManager();
    openOverlay('languagesOverlay');
  }

  renderLanguageManager(){
    const prefs=getState().prefs;
    const enabled=new Set(prefs.enabledLanguages);
    $('languageManagerGrid').innerHTML=courses.map(course=>{
      const isEnabled=enabled.has(course.id);
      const primary=prefs.primaryLanguage===course.id;
  ...
  handleLanguageManagerClick(event){
    ...
    const prefs=getState().prefs;
    let enabled=[...prefs.enabledLanguages];
    ...
    if(action==='primary'){
      ...
      updatePrefs({primaryLanguage:code,enabledLanguages:enabled});
      this.renderLanguageManager();
      return;
    }
    updatePrefs({enabledLanguages:enabled});
    this.renderLanguageManager();
  }
}
