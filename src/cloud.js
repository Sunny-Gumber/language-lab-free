import{applyRemotePrefs,getState,markEventsSynced,markPrefsSynced,mergeEvents,setScope,subscribe,unsyncedEvents}from'./store.js';
...
function profilePayload(){
  ...
  return{
    ...
    enabled_languages:prefs.enabledLanguages,
    audio_preference:prefs.audioPreference,
    daily_goal_xp:prefs.dailyGoalXp,
    onboarding_completed:prefs.onboardingCompleted,
    timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC'
  };
}

function eventToCloud(event,userId){
  return{
    id:event.id,
    user_id:userId,
    ...
    xp_delta:Number(event.xpDelta||0),
...
async function pullProfile(userId){
  const{data,error}=await client
    .from('profiles')
    .select('id,display_name,avatar_url,timezone,primary_language,enabled_languages,audio_preference,daily_goal_xp,onboarding_completed,updated_at')
    .eq('id',userId)
    .maybeSingle();
  if(error)throw error;
  if(data&&!getState().prefs.dirty)applyRemotePrefs(data);
  return data;
}
...
async function pullEvents(userId){
  ...
  for(let start=0;;start+=PAGE_SIZE){
    const{data,error}=await client
      ...
      .select('id,language_code,target_id,activity,skill,score,xp_delta,study_date,client_created_at,created_at,metadata')
      .eq('user_id',userId)
      .order('created_at',{ascending:true})
      .range(start,start+PAGE_SIZE-1);
    if(error)throw error;
    const rows=data||[];
    collected.push(...rows.map(eventFromCloud));
...
async function pushPreferences(userId){
  if(!getState().prefs.dirty)return false;
...
export async function syncNow(reason='manual'){
  ...
  try{
    ...
    console.warn('[Language Lab] cloud sync failed',error);
    setStatus(navigator.onLine?'☁ Sync pending':'Offline · saved locally');
  }finally{syncing=false}
}

const queueSync=debounce(()=>syncNow('local-change'),900);

function stopRealtime(){
  if(realtime&&client){try{client.removeChannel(realtime)}catch{}}
  realtime=null;
}
...
function startRealtime(userId){
  ...
  realtime=client.channel(`language-lab-v11-${userId}`)
    .on('postgres_changes',{event:'INSERT',schema:'public',table:'learning_events',filter:`user_id=eq.${userId}`},payload=>{
      if(payload.new)mergeEvents([eventFromCloud(payload.new)]);
    })
    .on('postgres_changes',{event:'UPDATE',schema:'public',table:'profiles',filter:`id=eq.${userId}`},payload=>{
      if(payload.new&&!getState().prefs.dirty)applyRemotePrefs(payload.new);
    })
    .subscribe();
}
...
async function activateSession(nextSession){
  ...
  try{
    ...
    console.warn('[Language Lab] account initialization failed',error);
    setStatus(navigator.onLine?'☁ Sync pending':'Offline · saved locally');
  }
...
export async function initializeCloud(){
  ...
  if(!globalThis.supabase?.createClient){
    setScope('guest');
    setStatus('Guest · cloud unavailable');
    for(const listener of authListeners)listener(null);
    return;
  }

  client=globalThis.supabase.createClient(SUPABASE_URL,SUPABASE_KEY,{
    auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}
  });

  const{data,error}=await client.auth.getSession();
  if(error)console.warn('[Language Lab] session read failed',error);
  ...
  client.auth.onAuthStateChange((event,nextSession)=>{
    ...
    activateSession(nextSession).catch(error=>console.warn('[Language Lab] auth state update failed',error));
  });

  subscribe((_,reason)=>{
    if(!session?.user)return;
    if(['event','prefs','position','voice'].includes(reason))queueSync();
  });
...
export function displayName(user=getUser()){
  if(!user)return'Guest';
  const metadata=user.user_metadata||{};
  return metadata.full_name||metadata.name||(user.email?user.email.split('@')[0]:'Learner');
}
...
export async function signOut(){
  if(!client){await activateSession(null);return}
  const{error}=await client.auth.signOut();
  if(error)throw error;
  await activateSession(null);
}

export async function deleteCloudLearning(languageCode=null){
  const user=getUser();
  if(!client||!user)return;
  let query=client.from('learning_events').delete().eq('user_id',user.id);
