...
async function fetchEvents(userId){const collected=[];for(let start=0;;start+=PAGE_SIZE){const{data,error}=await client.from('learning_events').select('id,language_code,target_id,activity,skill,score,xp_delta,study_date,client_created_at,created_at,metadata').eq('user_id',userId).order('created_at',{ascending:true}).range(start,start+PAGE_SIZE-1);if(error)throw error;const rows=data||[];collected.push(...rows.map(eventFromCloud));if(rows.length<PAGE_SIZE)break}return collected}
async function fetchPositions(userId){const{data,error}=await client.from('course_positions').select('language_code,unit_index,item_index,updated_at').eq('user_id',userId);if(error)throw error;return data||[]}

async function pushPreferences(userId,generation){if(!getState().prefs.dirty)return false;const payload=profilePayload(),{error}=await client.from('profiles').update(payload).eq('id',userId);if(error)throw error;if(isCurrent(userId,generation))markPrefsSynced();return true}
async function pushEvents(userId,generation){const pending=unsyncedEvents();if(!pending.length)return 0;for(let start=0;start<pending.length;start+=200){const batch=pending.slice(start,start+200),rows=batch.map(event=>eventToCloud(event,userId));const{error}=await client.from('learning_events').upsert(rows,{onConflict:'id',ignoreDuplicates:true});if(error)throw error;if(!isCurrent(userId,generation))return start+batch.length;markEventsSynced(batch.map(event=>event.id))}return pending.length}
async function pushPositions(userId,generation){const pending=dirtyPositions();if(!pending.length)return 0;const rows=pending.map(position=>({user_id:userId,language_code:position.languageCode,unit_index:position.unitIndex,item_index:position.itemIndex}));const{error}=await client.from('course_positions').upsert(rows,{onConflict:'user_id,language_code'});if(error)throw error;if(isCurrent(userId,generation))markPositionsSynced(pending.map(position=>position.languageCode));return pending.length}
...
export async function syncNow(reason='manual'){
  ...
  try{
    ...
    await pushPositions(userId,generation);if(!isCurrent(userId,generation))return;
    const[profile,events,positions]=await Promise.all([fetchProfile(userId),fetchEvents(userId),fetchPositions(userId)]);if(!isCurrent(userId,generation))return;
    if(profile&&!getState().prefs.dirty)applyRemotePrefs(profile);mergeEvents(events);applyRemotePositions(positions);
...
function startRealtime(userId,generation){
  ...
  realtime=client.channel(`language-lab-v11-${userId}`)
    .on('postgres_changes',{event:'INSERT',schema:'public',table:'learning_events',filter:`user_id=eq.${userId}`},payload=>{if(isCurrent(userId,generation)&&payload.new)mergeEvents([eventFromCloud(payload.new)])})
    .on('postgres_changes',{event:'UPDATE',schema:'public',table:'profiles',filter:`id=eq.${userId}`},payload=>{if(isCurrent(userId,generation)&&payload.new&&!getState().prefs.dirty)applyRemotePrefs(payload.new)})
...
async function activateSession(nextSession){
  ...
  const user=session.user,userId=user.id;setScope(`user:${userId}`);setStatus('⟳ Loading account');
