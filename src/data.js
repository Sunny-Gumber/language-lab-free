...
function itemId(language,unit,item){
  return `item:${language.id}:${hashString(`${unit.title}|${item.native}|${item.roman}`)}`;
}

function vocabId(language,word){
  return `vocab:${language.id}:${hashString(`${word.native}|${word.roman}|${word.meaning}`)}`;
}

export const courses=rawCourses.map(language=>{
  language.units=(language.units||[]).map((unit,unitIndex)=>{
    unit.id=`unit:${language.id}:${hashString(unit.title||String(unitIndex))}`;
    unit.index=unitIndex;
    unit.items=(unit.items||[]).map((item,itemIndex)=>{
      item.id=itemId(language,unit,item);
  ...
  language.vocab=(language.vocab||[]).map((word,index)=>{
    word.id=vocabId(language,word);
    word.index=index;
    return word;
  });
...
export function getItem(course,unitIndex,itemIndex){
  const unit=getUnit(course,unitIndex);
  return unit.items[Math.max(0,Math.min(Number(itemIndex)||0,unit.items.length-1))];
}

export function findItem(course,targetId){
  for(const unit of course.units){
    const item=unit.items.find(candidate=>candidate.id===targetId);
    if(item)return{unit,item,unitIndex:unit.index,itemIndex:item.index};
  }
  return null;
}
...
export function availableStages(course){
  ...
  if(stages.length)return stages.map(stage=>({
    ...stage,
    startUnit:Math.max(0,Number(stage.startUnit)||0),
    endUnit:Math.min(course.units.length-1,Number(stage.endUnit??course.units.length-1))
  }));
...
export function stageForUnit(course,unitIndex){
  return availableStages(course).find(stage=>unitIndex>=stage.startUnit&&unitIndex<=stage.endUnit)||availableStages(course)[0];
}

export function unitsForStage(course,stageId){
  const stage=availableStages(course).find(candidate=>candidate.id===stageId);
  if(!stage)return course.units;
  return course.units.slice(stage.startUnit,stage.endUnit+1);
}

function exampleTarget(item){
  if(!item.example?.native||!item.example?.meaning)return null;
  return{
    id:item.id,
    native:item.example.native,
    roman:item.example.roman||item.roman||'',
...
function vocabTarget(word){
  return{id:word.id,native:word.native,roman:word.roman||'',meaning:word.meaning,source:'vocab',word};
}
...
export function practiceTargets(course,skill,stageId=null){
  ...
  const vocabTargets=course.vocab.map(vocabTarget);

  let raw;
  if(skill==='writing')raw=units.flatMap(unit=>unit.items.map(item=>({
    id:item.id,
    native:item.native,
    roman:item.roman||'',
    meaning:item.example?.meaning||item.guide||'',
    source:'item',
    item
  })));
  else if(skill==='recognition'||skill==='recall')raw=vocabTargets;
  else raw=[...itemTargets,...vocabTargets];

  const seen=new Set();
  return raw.filter(target=>{
    const key=`${target.id}|${target.native}|${target.meaning}`;
    if(!target.native||seen.has(key))return false;
    seen.add(key);
    return true;
  });
}
...
export function allTargetIds(course){
  return unique([
    ...course.units.flatMap(unit=>unit.items.map(item=>item.id)),
    ...course.vocab.map(word=>word.id)
  ]);
}
