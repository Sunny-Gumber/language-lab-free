...
export class HomeController{
  constructor({openCourse,openPractice}){this.openCourse=openCourse;this.openPractice=openPractice;this.bind()}
  bind(){
    ...
    document.querySelector('.goal-buttons').addEventListener('click',event=>{const button=event.target.closest('[data-goal]');if(button)updatePrefs({dailyGoalXp:Number(button.dataset.goal)})});
    $('languageGrid').addEventListener('click',event=>{const button=event.target.closest('[data-language]');if(button)this.openCourse(button.dataset.language)});
  }
  ...
  render(){
    const state=getState(),primary=getCourse(state.prefs.primaryLanguage),xp=totalXp(),activeStreak=streak();
    ...
    this.renderDaily(primary);this.renderLanguages();this.renderGoal();
  }
  renderDaily(primary){
  ...
  renderLanguages(){
    const signedIn=isSignedIn();$('languagesHeading').textContent=signedIn?'My languages':'Choose a language';$('languagesDescription').textContent=signedIn?'Only courses you add appear here. Your primary language drives today’s practice.':'Guest mode shows all available courses.';$('addLanguageBtn').classList.toggle('hidden',!signedIn);$('manageLanguagesBtn').classList.toggle('hidden',!signedIn);
    const primary=getState().prefs.primaryLanguage;
    $('languageGrid').innerHTML=this.visibleCourses().map(course=>{const mastery=overallMastery(course.id);return`<button class="language-card" data-language="${course.id}" type="button"><div class="lang-top"><span class="lang-flag">${course.flag}</span>${signedIn&&course.id===primary?'<span class="primary-label">Primary</span>':`<span class="badge">${escapeHtml(course.level)}</span>`}</div><h3>${escapeHtml(course.name)}</h3><p>${escapeHtml(course.description)}</p><div class="progressbar"><span style="width:${mastery}%"></span></div><div class="lang-bottom"><span>${mastery}% overall</span><span>${course.units.length} units</span></div></button>`}).join('');
  }
}
