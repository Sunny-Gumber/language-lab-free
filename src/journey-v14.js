  ...
  renderJourney(){
    ...
    $('journeyTab').innerHTML=`
      ...
      <section class="learning-loop-v14">
        <div><b>1</b><span>Understand a situation</span></div><div><b>2</b><span>Hear connected language</span></div><div><b>3</b><span>Learn useful forms</span></div><div><b>4</b><span>Retrieve after a short delay</span></div><div><b>5</b><span>Read in context</span></div><div><b>6</b><span>Respond in your own words</span></div>
      </section>
      <section class="session-preview-v14"><div><span class="eyebrow">Recommended session</span><h3>${experience.mix.review} review · ${experience.mix.introduced} new targets</h3><p>${escapeHtml(experience.mix.label)} · interleaved retrieval${experience.dialogue.length?' · connected dialogue':''}${experience.reading?.native?' · reading':''}${experience.checkpoint?' · stage checkpoint':''}</p></div><div><span class="eyebrow">Course depth</span><h3>${escapeHtml(depth.label)}</h3><p>${escapeHtml(depth.detail)}</p></div></section>
