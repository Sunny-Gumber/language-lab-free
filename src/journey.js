...
const lessonState=(unitIndex,itemIndex)=>({unitIndex,itemIndex,step:0,answered:false,answerCorrect:null,selectedAnswer:null,options:null,speakingDone:false,speakingMessage:''});
...
export class JourneyController{
  ...
  open(languageCode,unitIndex=null){
    this.course=getCourse(languageCode);this.lesson=null;
    const path=this.path(),requested=unitIndex==null?getPosition(this.course.id).unitIndex:Number(unitIndex)||0;
    if(!path[requested]?.unlocked)unitIndex=this.recommendedIndex(path);
    this.render('journey');
  }
  ...
  unitMetric(unit){
    ...
    return{coverage,activeMastery,readiness,complete:coverage>=80&&readiness>=60,allowNext:coverage>=60&&readiness>=45,activeEvidence};
  }
  ...
  path(){
    ...
    return this.course.units.map((unit,index)=>{
      ...
      priorReady=unlocked&&metric.allowNext;
      return{unit,index,metric,unlocked};
    });
  }
  recommendedIndex(path=this.path()){
    const first=path.find(entry=>entry.unlocked&&!entry.metric.complete);if(first)return first.index;
    return path.filter(entry=>entry.unlocked).at(-1)?.index||0;
  }
  courseProgress(path=this.path()){
    if(!path.length)return 0;
    return Math.round(path.reduce((sum,entry)=>sum+entry.metric.readiness,0)/path.length);
  }
  ...
  handleJourneyClick(event){
    ...
    if(start){this.startLesson(Number(start.dataset.journeyStart??start.dataset.journeyUnit)||0);return}
    ...
    if(answer){this.answerLessonCheck(answer);return}
  ...
  startLesson(unitIndex){
    ...
    this.lesson=lessonState(unitIndex,firstNew>=0?firstNew:0);
  ...
  renderLesson(){
    ...
    if(step===2){
      const options=this.checkOptions(item),feedback=!this.lesson.answered?'Choose one answer.':this.lesson.answerCorrect?`✅ Correct — <b>${escapeHtml(phrase)}</b> means ${escapeHtml(meaning)}.`:`Not yet. The answer is <b>${escapeHtml(meaning)}</b>. Listen once more before moving on.`;
      body=`<div class="guided-focus-v13"><span class="eyebrow">Listen and choose the meaning</span><button class="guided-play-round" data-lesson-action="play" type="button">🔊</button><p class="muted">Use the sound, not just the spelling.</p></div><div class="guided-options-v13">${options.map(option=>{const correct=option===meaning,selected=option===this.lesson.selectedAnswer,classes=this.lesson.answered?(correct?'correct':selected?'wrong':''):'';return`<button class="${classes}" data-lesson-answer="${escapeHtml(option)}" type="button" ${this.lesson.answered?'disabled':''}>${escapeHtml(option)}</button>`}).join('')}</div><div id="guidedFeedback" class="practice-feedback">${feedback}</div>${this.lesson.answered?'<button class="primary full" data-lesson-action="continue" type="button">Now say it →</button>':''}`;
    }
    if(step===3){const message=this.lesson.speakingMessage||'Speak after the model. Browser scoring is transcript matching, not accent or phoneme scoring.';body=`<div class="guided-focus-v13"><span class="eyebrow">Use it aloud</span><div class="guided-native-v13" dir="${this.course.rtl?'rtl':'ltr'}">${escapeHtml(phrase)}</div>${roman?`<b class="guided-roman-v13">${escapeHtml(roman)}</b>`:''}<p>${escapeHtml(meaning)}</p><div class="actions"><button class="secondary" data-lesson-action="play" type="button">🔊 Hear again</button><button class="primary" data-lesson-action="mic" type="button">🎙 Check with microphone</button></div><button class="ghost" data-lesson-action="manual-speak" type="button">I practised it aloud without microphone</button><div id="guidedFeedback" class="practice-feedback">${message}</div></div>${this.lesson.speakingDone?'<button class="primary full" data-lesson-action="continue" type="button">Finish this item →</button>':''}`}
  ...
  answerLessonCheck(button){
    ...
    this.lesson.answered=true;this.lesson.answerCorrect=correct;this.lesson.selectedAnswer=selected;
  ...
  recordManualSpeaking(){
    ...
    this.lesson.speakingDone=true;this.lesson.speakingMessage='Speaking practice recorded as coverage. No accuracy score was assigned.';this.renderLesson();
  }

  recognizeLesson(){
    const item=this.lessonItem(),Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;
    if(!Recognition){this.lesson.speakingMessage='Microphone speech recognition is unavailable here. Use the practice-aloud option instead.';this.renderLesson();return}
    ...
    recognizer.onend=()=>{const current=$('journeyTab')?.querySelector('[data-lesson-action="mic"]');if(current){current.disabled=false;current.textContent='🎙 Check with microphone'}};
    recognizer.onresult=event=>{
      ...
      recordPractice({languageCode:this.course.id,targetId:item.id,skill:'speaking',score,xp,metadata:{mode:'guided-journey',heard}});
  ...
  nextLessonItem(){
    ...
    this.lesson=lessonState(this.lesson.unitIndex,next);setPosition(this.course.id,this.lesson.unitIndex,next);this.renderLesson();
  }
